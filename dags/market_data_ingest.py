...
def market_data_ingest():
    ...
    @task
    def fetch_and_store_prices(symbol: str) -> dict[str, int | str]:
        import requests
        from airflow.providers.snowflake.hooks.snowflake import SnowflakeHook
        ...
        if len(bars) < MIN_REQUIRED_BARS:
            raise ValueError(
                f"{symbol} returned {len(bars)} daily bars; at least "
                f"{MIN_REQUIRED_BARS} are required for a reliable SMA-200"
            )
    ...
    @task
    def fetch_and_store_news(symbol: str) -> dict[str, int | str]:
        import requests
        from airflow.providers.snowflake.hooks.snowflake import SnowflakeHook
        ...
        return {"symbol": symbol, "articles_loaded": len(rows)}

    @task(outlets=[STOCK_PRICES_ASSET, STOCK_NEWS_ASSET])
    def publish_market_data(price_results: list[dict], news_results: list[dict]) -> None:
        total_bars = sum(result["rows_loaded"] for result in price_results)
        total_articles = sum(result["articles_loaded"] for result in news_results)
        print(
            f"Market data refresh complete: {total_bars} price rows and "
            f"{total_articles} news articles across {len(price_results)} tickers"
        )
